*&---------------------------------------------------------------------*
*& Report ymay_oia_basic
*&---------------------------------------------------------------------*
*&
*&---------------------------------------------------------------------*
REPORT ymay_oia_aggregated.
...
select bpa~bp_id, bpa~company_name, head~changed_at,
       count( * ) as inv_count from snwd_so_inv_head as head
              inner join snwd_bpa as bpa
              on head~buyer_guid = bpa~node_key
              where head~payment_status = ''
              group by bpa~bp_id, bpa~company_name, head~changed_at
              into @data(ls_dats).

    "convert timestamp to date
    CONVERT TIME STAMP ls_dats-changed_at TIME ZONE sy-zonlo
                       INTO DATE data(lv_date).

    "Since how long its not paid
    ls_open_days-days_due = ( sy-datum - lv_date ) * ls_dats-inv_count.
    ls_open_days-inv_count = ls_dats-inv_count.
    ls_open_days-bp_id = ls_dats-bp_id.
    ls_open_days-company_name = ls_dats-company_name.
    COLLECT ls_open_days into lt_open_days.

ENDSELECT.

""Step 3: Calculate the total amount due in common currency
select bpa~bp_id, sum( itm~gross_amount ) as gross_amount,
                  itm~currency_code from snwd_so_inv_head as head
    ...
    ls_open_amount-bp_id = ls_amount-bp_id.

    if ( ls_cust-currency_code <> ls_amount-currency_code ).
        call FUNCTION 'CONVERT_TO_LOCAL_CURRENCY'
          EXPORTING
*            client            = SY-MANDT
            date              = sy-datum
            foreign_amount    = ls_amount-gross_amount
            foreign_currency  = ls_amount-currency_code
